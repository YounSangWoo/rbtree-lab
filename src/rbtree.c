...
node_t *grandparent(rbtree *t, node_t *x)
{
...
/*  int main()
{
  ...
  int length = sizeof(arr1) / sizeof(key_t);
  key_t *res1 = calloc(length, sizeof(key_t));
  rbtree *t = new_rbtree();

  for (int i = 0; i < length; i++)
  {
    rbtree_insert(t, arr1[i]);
  }

  //preorder(t->root);
  inorder(t, t->root, res1);
  // for (int i = 0; i < length; i++)
  // {
  //   printf("res1 key = %d\n", res1[i]);
  // }
